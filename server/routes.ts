...
export async function registerRoutes(
  ...
  // Explainer presentation video endpoints
  app.get("/api/video/explainer-presentation-status", async (req: Request, res: Response) => {
    try {
      const { getExplainerVideoPath } = await import("./services/explainerRecorder");
  ...
  app.post("/api/video/record-explainer-presentation", async (req: Request, res: Response) => {
    try {
      const { recordExplainerVideo } = await import("./services/explainerRecorder");
      const protocol = req.headers['x-forwarded-proto'] || 'http';
      const host = req.headers.host || 'localhost:5000';
      const baseUrl = `${protocol}://${host}`;
      
      console.log(`[ExplainerRecorder] Starting recording with base URL: ${baseUrl}`);
      const outputPath = await recordExplainerVideo(baseUrl);
      res.json({ success: true, message: "Video recorded successfully", path: outputPath });
    } catch (err: any) {
      console.error("[ExplainerRecorder] Error:", err.message);
      res.status(500).json({ success: false, message: err.message });
    }
  });

  app.get("/api/video/explainer-presentation", async (req: Request, res: Response) => {
    try {
      const { getExplainerVideoPath } = await import("./services/explainerRecorder");
      const videoPath = getExplainerVideoPath();
      
      if (!videoPath) {
        return res.status(404).json({ message: "Explainer presentation video not found. Generate it first." });
      }
      ...
      const filename = "BitForce_Explainer_Presentation.mp4";
  ...
  app.delete("/api/video/explainer-presentation", async (req: Request, res: Response) => {
    try {
      const { deleteExplainerVideo } = await import("./services/explainerRecorder");
      const deleted = await deleteExplainerVideo();
      res.json({ success: deleted, message: deleted ? "Video deleted" : "No video to delete" });
    } catch (err: any) {
      res.status(500).json({ success: false, message: err.message });
    }
  });
