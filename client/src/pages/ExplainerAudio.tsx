...
import explainerAudio from "@assets/explaineraudio_1768833558627.mp3";
...
export default function ExplainerAudio() {
  const [isPlaying, setIsPlaying] = useState(false);
  const [isMuted, setIsMuted] = useState(false);
  const [currentTime, setCurrentTime] = useState(0);
  const [currentScene, setCurrentScene] = useState(0);
  const [audioDuration, setAudioDuration] = useState(TOTAL_DURATION);
  ...
  useEffect(() => {
    const timeRatio = currentTime / audioDuration;
    const animationTime = timeRatio * TOTAL_DURATION;
    
    let elapsed = 0;
    let foundScene = false;
    for (let i = 0; i < scenes.length; i++) {
      if (animationTime >= elapsed && animationTime < elapsed + scenes[i].duration) {
        setCurrentScene(i);
        foundScene = true;
        break;
      }
    ...
    if (!foundScene && animationTime >= TOTAL_DURATION) {
  ...
  const getSceneProgress = () => {
    ...
    const sceneTime = Math.max(0, animationTime - elapsed);
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-slate-900">
      <audio ref={audioRef} src={explainerAudio} preload="auto" />
      ...
      {!isPlaying && currentTime === 0 && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/50 z-20">
          <Button
            size="lg"
            className="bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 text-white px-8 py-6 text-xl rounded-xl gap-3"
            onClick={handlePlayPause}
